...
const chainId = Theta.networks.ChainIds.Testnet;
const theta_provider = new Theta.providers.HttpProvider(chainId);
...
async function balanceOf(userAddress) {
    const BATContract = new Theta.Contract(contractAddress, abi, theta_provider);
    const balance = await BATContract.balanceOf(userAddress);
    return balance;
}

async function engagementScores(userAddress) {
    const BATContract = new Theta.Contract(contractAddress, abi, theta_provider);
    const engagement_scores = await BATContract.engagementScores(userAddress);
    return engagement_scores;
}

async function referralScores(userAddress) {
    const BATContract = new Theta.Contract(contractAddress, abi, theta_provider);
    const referral_scores = await BATContract.referralScores(userAddress);
    return referral_scores;
}

async function leadGenerationScores(userAddress) {
    const BATContract = new Theta.Contract(contractAddress, abi, theta_provider);
    const lead_generation_scores = await BATContract.leadGenerationScores(userAddress);
    return lead_generation_scores;
}

async function viewScores(userAddress) {
    const BATContract = new Theta.Contract(contractAddress, abi, theta_provider);
    const view_scores = await BATContract.viewScores(userAddress);
    return view_scores;
}

async function contributionScores(userAddress) {
    const BATContract = new Theta.Contract(contractAddress, abi, theta_provider);
    const contribution_scores = await BATContract.contributionScores(userAddress);
